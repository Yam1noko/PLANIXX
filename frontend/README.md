...
- default cookie path is `/api/auth`
- by default cookie domain is not set, so the cookie is host-only
...
### Local Dev Host Agreement
For local browser development, use one host consistently for both frontend and backend auth traffic.
...
- one host for `login` and another host for `refresh`

Why:
- refresh cookie is host-only by default because `AUTH_REFRESH_COOKIE_DOMAIN` is unset
- a cookie issued for `localhost` will not be sent to `127.0.0.1`
- after moving local setup to Docker, this is the most common reason for unexpected `401` on `/api/auth/refresh`

Local Docker dev command:
...
- active run-level settings are `mode`, `max_schedule_variants`, `use_warm_start`, and `replan_from_current_schedule`
- `max_daily_planned_minutes` is kept in the request schema for compatibility, but is not enforced as a hard scheduling constraint during solve
...
- scheduler result status is always `success` or `partial`; `failed` is not emitted
- if strict placement is impossible, backend degrades to best-effort placement and may leave some mandatory tasks unscheduled instead of failing the whole run
...
Notes:
- preview returns the full in-memory `planning_result`
- it does not save a new current schedule
...
Notes:
- this endpoint returns a compacted `planning_result` to avoid huge response payloads
- `planning_result.source_request` is intentionally omitted in this response
- only one variant is returned in `planning_result.schedule_variants`
- the persisted schedule can be read directly from `current_schedule`
...
8. Then one of:
   - `POST /api/users/{user_id}/schedule-from-stored-data`
   - `POST /api/users/{user_id}/best-schedule-from-stored-data`
9. If you used `schedule-from-stored-data`, you can read the saved schedule directly from `current_schedule` in the response
10. Or read saved result separately:
   - `GET /api/users/{user_id}/schedules/current`
